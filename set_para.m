classdef set_para
    properties(Constant)
        ...
        c  = 1540;                          % [m/s]
        lambda = set_para.c / set_para.f0;

        %% ===================== ELEMENT GEOMETRY (BASE) =====================
        height = 8.5e-3;                    % [m]
        kerf   = 0.01e-3;                   % [m]

        %% ===================== PULSE =====================
        cycles = 2;
        t = 0:1/set_para.fs:set_para.cycles/set_para.f0;
        excitation = sin(2*pi*set_para.f0*set_para.t);
        impulse_response = set_para.excitation .* ...
                           hanning(numel(set_para.excitation))';
        pulse_length = set_para.cycles * set_para.lambda;
    ...
    methods(Static)
        function settings = get(case_name, overrides)
            ...
            settings.element_position = settings.element_position(:);

            %% ---- image grid ----
            settings.dx = set_para.dx;
            settings.dz = set_para.dz;
            settings.num_z_pixel = set_para.num_z_pixel;
            settings.img_start   = set_para.img_start;

            settings.img_x_border = [-settings.aperture_size/2, ...
                                      settings.aperture_size/2];
            settings.img_z_border = [ settings.img_start/1000, ...
                                      settings.img_start/1000 + ...
                                      settings.num_z_pixel*settings.dz ];

            if isfield(overrides,'img_x_border')
                settings.img_x_border = overrides.img_x_border;
            end
            if isfield(overrides,'img_z_border')
                settings.img_z_border = overrides.img_z_border;
            end
